...
public class Security extends WebSecurityConfigurerAdapter {
	...
	// New organiser and attendee roles
	public static final String ORG_ROLE = "ORGANISER";
	...
	private static final RequestMatcher[] NO_AUTH = {
			...
			new AntPathRequestMatcher("/events/{id:[0-9]+}", "POST")};
	
	// Actions allowed for organiser role
	private static final RequestMatcher[] ORG_AUTH = {
			...
			new AntPathRequestMatcher("/api/venues/**", "POST"),
			new AntPathRequestMatcher("/api/venues/**", "DELETE")};
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().requestMatchers(NO_AUTH).permitAll()
		                        .requestMatchers(ORG_AUTH).hasAnyRole(ORG_ROLE, ADMIN_ROLE)
	...
	public UserDetailsService userDetailsService() {
		...
		// Organiser login
		UserDetails org = User.withUsername("Organiser").password(encoder.encode("password")).roles(ORG_ROLE).build();

		return new InMemoryUserDetailsManager(rob, caroline, markel, mustafa, tom, org);
	}
}
