...
public class EventsControllerApiIntegrationTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void getEvent() {
		client.get().uri("/events/1").accept(MediaType.APPLICATION_JSON).exchange().expectStatus().isOk()
				...
				.jsonPath("$._links.self.href").value(endsWith("/1"))
				.jsonPath("$.name").isEqualTo("Software Engineering Lecture");
	}
	
	@Test
	public void testSearchEventsByName() {
		String testEventName = "1";
		client.get().uri("/events/search?nameSearch="+testEventName).accept(MediaType.APPLICATION_JSON).exchange().expectStatus().isOk().expectHeader()
				.contentType(MediaType.APPLICATION_JSON).expectBody().jsonPath("$._links.self.href")
				.value(endsWith("/api/events/search?nameSearch="+testEventName)).jsonPath("$._embedded.events.length()").value(equalTo(1));
		testEventName = "Soft";
		client.get().uri("/events/search?nameSearch="+testEventName).accept(MediaType.APPLICATION_JSON).exchange().expectStatus().isOk().expectHeader()
		.contentType(MediaType.APPLICATION_JSON).expectBody().jsonPath("$._links.self.href")
		.value(endsWith("/api/events/search?nameSearch="+testEventName)).jsonPath("$._embedded.events.length()").value(equalTo(3));
	}
}
